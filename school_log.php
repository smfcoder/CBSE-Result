<?php
   ...
   if(isset($_POST['submit'])){
        $login = $_POST['user'];
        $pass = $_POST['pass'];
        $db="SELECT * FROM school WHERE username='$login' and pass ='$pass'";
        $result=mysqli_query($sql, $db);
        $row = mysqli_fetch_array($result);
        if($row['username']==$login && $row['pass']==$pass)
        {
            $_SESSION['schoolId']=$login;   
            header('Location:school/school_af_log.php');
            exit();
       }
       else{
...
<html lang="en">
  ...
  <body class="text-center">
    ...
    <form action="" method="post">
        ...
        <img src="img/school_log.jpg"></img></div>
