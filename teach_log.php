<?php
   ...
   if(isset($_POST['submit'])){
        $login = $_POST['user'];
        $pass = $_POST['pass'];
       $db="SELECT * FROM teacher WHERE username='$login' and pass ='$pass'";
       ...
       if($row['username']==$login && $row['pass']==$pass)
       {
            if($row['class']!="NA")
            {
       		    $_SESSION['teachId']=$login;
       		    $_SESSION['classId']=$row['class'];
                header('Location:teacher/teacher_af_log.php');
                exit();
            }
            else{
                echo '<script type="text/javascript">alert("Any class is not Alloted to you  (: ,Contact Admin");</script>';
            }
       }
       else{
...
<html lang="en">
  ...
  <body class="text-center">
    ...
    <form action="" method="post">
        ...
        <img src="img/teacherlogin.png"></img></div>
