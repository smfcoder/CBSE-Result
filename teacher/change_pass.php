...
<?php
  ...
  if($pwd == $c_pwd) :
  if($pwd!=$old_pwd) :
    $sql="SELECT * FROM `teacher` WHERE `username`='".$sname."' AND `pass` ='$old_pwd'";
    $db_check=$db->query($sql);
    $count=mysqli_num_rows($db_check);
   
  if($count==1) :
    $fetch=$db->query("UPDATE `teacher` SET `pass` = '".$pwd."' WHERE `username`='".$sname."' ");
    $old_password=''; $password =''; $confirm_pwd = '';
    $_SESSION['passupdate'] = 'success';
    $msg_sucess = "<script>alert('Your new password updated successfully.')</script>";
    ...
    $error = "<script>alert('Please fill all the fields.')</script>";
